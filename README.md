...
```
PowerShell -executionpolicy bypass -command "wget https://github.com/DesktopECHO/kWSL/raw/devuan/kWSL.cmd -UseBasicParsing -OutFile kWSL.cmd ; .\kWSL.cmd"
```
...
- Edit ```kWSL.cmd```.  Near the top you will see ```SET GITORG=DesktopECHO``` - Change ```DesktopECHO``` to the name of your own repository and change BRANCH=devuan to ```master```.
